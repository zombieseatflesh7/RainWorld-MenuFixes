...
public static class OptimizedRemix
{
    ...
    public static void AddHooks()
    {
        try
        {
            greyscaleShader = RWCustom.Custom.rainWorld.Shaders["MenuFixes.Greyscale"];

            // Prevent thumbnails from loading all at once
            On.Menu.Remix.ConfigContainer.QueueModThumbnails += (orig, self, buttons) => { };
            ...
            On.Menu.Remix.MenuModList.ModButton.ctor += (orig, self, list, index) =>
            {
                ...
                catch (Exception e) { Plugin.Logger.LogError(e.Message + "\n" + e.StackTrace); }
            };
    ...
    {
        Plugin.Logger.LogInfo($"Loading mod thumbnail: {button.itf.mod.id}");
